...
class LSTMModel(BaseModel):
    ...
    def _train_word2vec(self, train_data: List[Tuple[str, int]], **kwargs):
        ...
        print(f"Word2Vec training completed, word vector dimension: {vector_size}")
    ...
    def predict(self, texts: List[str]) -> List[int]:
        ...
        # Create dataset
        test_data = [(text, 0) for text in texts]  # Labels don't matter
    ...
    def predict_single(self, text: str) -> Tuple[int, float]:
        ...
        if len(vectors) == 0:
            return 0, 0.5  # If no valid word vectors, return default value
